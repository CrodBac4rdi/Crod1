...
export class LayeredAtomicManager {
    ...
    async storeBaseAtom(wingPath, atomType, tags, initialWeight = 1.0) {
        // Validate inputs
    ...
    async addAtomReference(atomId, refType, refTarget, refStrength = 0.5) {
        const stmt = this.db.prepare(`
        ...
        const batchTransaction = this.db.transaction(() => {
            ...
            for (const atom of atoms) {
                ...
                const atomHash = crypto.createHash('sha256')
                    ...
                    .digest('hex');
                atomStmt.run(atomId, atomHash, atom.atomType, atom.initialWeight || 1.0, JSON.stringify(atom.wingPath));
    ...
    async createContext(atomId, contextType, adjustedWeight = 1.0) {
    ...
    async adjustContext(contextId, adjustmentType, adjustmentValue, reason) {
    ...
    async createPatternChain(chainName, chainType, atomIds) {
        // Validate inputs
    ...
    async validatePatternChain(chainId) {
        // Get chain members
        ...
        for (let i = 1; i < members.length; i++) {
            ...
            `).all(members[i - 1].atom_id);
            const hasConnection = prevRefs.some((ref) => ref.ref_target === members[i].atom_id);
            coherenceScore *= hasConnection ? 1.0 : 0.8;
        }
        // Calculate average accuracy from confidence scores
        const avgConfidence = members.reduce((sum, m) => sum + (m.confidence_score || 0.8), 0) / members.length;
    ...
    async refactorPatternChain(chainId, refactorType) {
        ...
        if (improved) {
            ...
            this.db.prepare(`
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            `).run(refactorId, chainId, refactorType, JSON.stringify(beforeState), JSON.stringify(afterState), 0.1); // Placeholder improvement score
        }
        return improved;
    }
    // Cross-layer query optimization
    async queryWithOptimization(query, layers = ['base', 'context', 'validation']) {
        ...
        if (layers.includes('base')) {
            ...
            results.push(...baseResults.map((r) => ({ ...r, layer: 'base' })));
        }
        ...
        if (layers.includes('context') && results.length > 0) {
            ...
            `).all(...atomIds);
            results.forEach((r) => {
                r.contexts = contextResults.filter((c) => c.atom_id === r.atom_id);
            });
        }
        ...
        if (layers.includes('validation') && results.length > 0) {
            ...
            `).all(...atomIds);
            results.forEach((r) => {
                r.patterns = patternResults.filter((p) => p.atom_id === r.atom_id);
            });
        }
        // Update heat map
        results.forEach((r) => {
    ...
    updateHeatMap(atomId, increment) {
        this.db.prepare(`
...
//# sourceMappingURL=layered-atomic-manager.js.map